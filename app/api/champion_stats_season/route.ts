...
/** GET: full season champion stats from matches table. Inputs: puuid, queueKey (solo|flex). No Riot calls. */
export async function GET(req: Request) {
  const { searchParams } = new URL(req.url);
  const puuid = searchParams.get("puuid") ?? "";
  const queueKey = (searchParams.get("queueKey") ?? searchParams.get("queue") ?? "solo") === "flex" ? "flex" : "solo";
  if (!puuid) return NextResponse.json({ rows: [] }, { status: 200 });

  const queueId = queueKey === "flex" ? QUEUE_FLEX : QUEUE_SOLO;
