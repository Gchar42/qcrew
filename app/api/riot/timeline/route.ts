...
import type { MatchTimelineDto } from "@/types/riot";
...
export async function GET(request: Request) {
  ...
  const cacheKey = `timeline:${region}:${id}`;
  ...
  const url = `${base}/${encodeURIComponent(id)}/timeline`;
  ...
  if (!res.ok) {
    console.error("[riot/timeline] Riot response:", res.status, res.statusText, text);
    const message = text || "Timeline fetch failed";
    return NextResponse.json(
  ...
  const data = JSON.parse(text) as MatchTimelineDto;
